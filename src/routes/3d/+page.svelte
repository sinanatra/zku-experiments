<script>
    ...
    function updateEnvironment(datum) {
        ...
        const rain = mapValue(datum.humidity, 0, 100, 0, 100);
        const light = mapValue(datum.solarradiation, 0, 1000, 0, 100);
        scene.background = new THREE.Color(`hsl(${adjustedHue}, 50%, ${light}%)`);
        

        let cloudColor = new THREE.Color(`hsl(0, 0%, ${-rain}%)`);

        const dewpoint = mapValue(datum.dewpoint, 0, 30, 0, 20);
        const humidity = mapValue(datum.humidity, 0, 100, 0, 20);
        ...
        updateClouds(cloudDensity, cloudColor);
        time = datum.time + " | " + cloudDensity;
    }

    function updateClouds(cloudCover, cloudColor) {
